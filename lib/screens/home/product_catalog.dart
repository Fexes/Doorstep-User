...
import 'package:Doorstep/models/CategoryName.dart';
...
class _ProductCatalogState extends State<ProductCatalog> {
  ...
  filterProductSearch(String filter) {
    ...
    final FirebaseDatabase database = FirebaseDatabase.instance;
    volunteerRef = database.reference().child("Shops").child(DataStream.ShopCatagory).child(shop.shopid).child("Products").child("Categories").child("ALL");
    volunteerRef.orderByChild("cardname").startAt(filter.toUpperCase()).endAt(filter.toLowerCase()).onChildAdded.listen(
            (event){
          isLoaded=true;

          if(Product.fromSnapshot(event.snapshot).cardname.toUpperCase().contains(filter.toUpperCase())){
  ...
  Future<void> loadmore(String last) async {
    ...
    loadMore=0;
    final FirebaseDatabase database = FirebaseDatabase.instance;
    volunteerRef = database.reference().child("Shops").child(DataStream.ShopCatagory).child(shop.shopid).child("Products").child("Categories").child(selectfilter);
  ...
  void setupcategoryNames(){
    ...
    final FirebaseDatabase database = FirebaseDatabase.instance;
    volunteerRef = database.reference().child("Shops").child(DataStream.ShopCatagory).child(shop.shopid).child("Products").child("CategoryName");
    volunteerRef.onChildAdded.listen((event) {
      setState(() {

        categoryNames.add(CategoryName.fromSnapshot(event.snapshot));
        if(isf) {
          isf=false;
          setuplist(categoryNames[0].category_name);
        }
      });
  ...
  Future<void> setuplist(String category) async {
    ...
    final FirebaseDatabase database = FirebaseDatabase.instance;
    volunteerRef = database.reference().child("Shops").child(DataStream.ShopCatagory).child(shop.shopid).child("Products").child("Categories").child(category);
    volunteerRef.orderByChild("cardname").onChildAdded.listen(
            (event) {
              ...
              Column(
                children: [
                  ...
                  SingleChildScrollView(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: categoryNames.map((CategoryName char){
                        return
                          Row(
                              ...
                              Padding(
                                ...
                                child: FlatButton(
                                  onPressed: (){
                                    ...
                                    selectfilter=char.category_name.toUpperCase();
                                    setState(() {

                                    });

                                    setuplist(char.category_name.toUpperCase());
                                  ...
                                  color:selectfilter==char.category_name? Colors.green[100]: Colors.white ,


                                  textColor: Colors.green[600],
                                  padding: EdgeInsets.all(8.0),
                                  child: Text(
                                    char.category_name.toUpperCase(),
                                    style: TextStyle(
                                      fontSize: 14.0,
                                    ),
                                  ),
